...
def fill_range(img):
    '''Given an image, stretches the RGB values so they take up the entire possible value range.'''
    img = img.astype(float)
    img -= np.min(img)
    img /= np.max(img)+1e-12
    return img

# TODO def add_border(imgs:np.array) -> np.array:

def tile(imgs, aspect_ratio=1.0, grid_shape=None, border=1, border_color=1) -> np.array:
    ''' 
    ...
    # Code to optionally stretch the images to fill the range, 
    # but this is unrelated to this method and should be called separately instead.
    # # Prepare images
    # if stretch:
    #         imgs = fill_range(imgs)
    # imgs = np.array(imgs)
...
# def tile_raster_images(X, img_shape, grid_shape, tile_spacing=(0, 0),
...
#     :type grid_shape: tuple; (rows, cols)
#     :param grid_shape: the number of images to tile (rows, cols)
...
#     #
#     # out_shape = [0,0]
#     # out_shape[0] = (img_shape[0] + tile_spacing[0]) * grid_shape[0] -
#     #                tile_spacing[0]
#     # out_shape[1] = (img_shape[1] + tile_spacing[1]) * grid_shape[1] -
#     #                tile_spacing[1]
#     out_shape = [(ishp + tsp) * tshp - tsp for ishp, tshp, tsp
#                                             in zip(img_shape, grid_shape, tile_spacing)]
...
#                             # in the output
#                             out_array[:, :, i] = tile_raster_images(X[i], img_shape, grid_shape, tile_spacing, scale_rows_to_unit_interval, output_pixel_vals)
#             return out_array
...
#                                             # function
#                                             this_img = scale_to_unit_interval(X[tile_row * grid_shape[1] + tile_col].reshape(img_shape))
#                                     else:
#                                             this_img = X[tile_row * grid_shape[1] + tile_col].reshape(img_shape)
