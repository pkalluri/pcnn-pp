"Create super simple unconditional dataset of blank (red or blue) images."
...
def create_dbg_dataset(n, image_dim=16):
    """
    n: number of images
    ...
    labels = []
    for i in range(n):
        draw_red = random.randint(0,1) == 0 # coin flip
        labels.append(int(draw_red))

        if draw_red:
            color = (255, 0, 0)
        else:
            color = (0, 0, 255)
        img = Image.new('RGB', (int(image_dim), int(image_dim)), color)
        imgs.append(np.array(img))
    # done making images
    return np.stack(imgs), np.array(labels) # xs have shape (N, d, d, c), ys have shape (N,)

def download_if_not_downloaded(dataset_dir, subset, force_download=False):
    ...
    if not os.path.exists(filepath) or force_download:
        subset2samples = {'train': 40, 'test': 40}
        x,y = create_dbg_dataset(n=subset2samples[subset])
        np.savez(filepath, x, y)

    return filename

def load_downloaded(dataset_dir, filename):
...
def load_DBG(data_dir, subset='train'):
    assert(subset in ('train', 'test'))
    dataset_dir = os.path.join(data_dir, 'DBG')
    filename = download_if_not_downloaded(dataset_dir, subset, force_download=True)
    x, y = load_downloaded(dataset_dir, filename)
...
class DataLoader(object):
    ...
    def __init__(self, data_dir, subset, batch_size, rng=None, shuffle=False, return_labels=False):
        ...
        data, labels = load_DBG(data_dir, subset=subset)
