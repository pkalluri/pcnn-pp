...
for i in range(args.nr_gpu):
    with tf.device('/gpu:%d' % i):
        ...
        # loss_gen_standard.append(loss_fun(xs[i], out, "log_prod", args.entropy))
        # loss_gen_sum.append(loss_fun(xs[i], out, "log_sum", args.entropy))
        # loss_gen_max.append(loss_fun(xs[i], out, "log_max", args.entropy))
...
bits_per_dim_standard = loss_gen_standard[0]/(args.nr_gpu*np.log(2.)*np.prod(obs_shape)*args.batch_size)
bits_per_dim_sum = loss_gen_sum[0]/(args.nr_gpu*np.log(2.)*np.prod(obs_shape)*args.batch_size)
bits_per_dim_max = loss_gen_max[0]/(args.nr_gpu*np.log(2.)*np.prod(obs_shape)*args.batch_size)
...
with tf.Session() as sess:
    for epoch in range(args.max_epochs):
        ...
        #     l,l_st,l_ = sess.run([bits_per_dim, bits_per_dim_standard, bits_per_dim_sum, bits_per_dim_max, optimizer], feed_dict)
        ...
        for d in train_data:
            ...
            # print(train_loss, standard_loss, sum_loss, max_loss)
            train_losses.append(train_loss)
            train_standard_losses.append(standard_loss)
            train_sum_losses.append(sum_loss)
            train_max_losses.append(max_loss)
        train_loss_gen = np.mean(train_losses)
        train_standard_loss_gen = np.mean(train_standard_losses)
        train_sum_loss_gen = np.mean(train_sum_losses)
        train_max_loss_gen = np.mean(train_max_losses)
        ...
        for d in test_data:
            feed_dict = make_feed_dict(d)
            test_loss, standard_loss, sum_loss, max_loss = sess.run([bits_per_dim_test, bits_per_dim_standard, bits_per_dim_sum, bits_per_dim_max], feed_dict)
            test_losses.append(test_loss)
            test_standard_losses.append(standard_loss)
            test_sum_losses.append(sum_loss)
            test_max_losses.append(max_loss)
        test_loss_gen = np.mean(test_losses)
        test_standard_loss_gen = np.mean(test_standard_losses)
        test_sum_loss_gen = np.mean(test_sum_losses)
        test_max_loss_gen = np.mean(test_max_losses)
        ...
        print("Iteration %d, time = %ds, \
                ...
                test_standard_loss_gen, test_sum_loss_gen, test_max_loss_gen,
                train_standard_loss_gen, train_sum_loss_gen, train_max_loss_gen))
        sys.stdout.flush()
